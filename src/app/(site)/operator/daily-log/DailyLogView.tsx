...
export default function DailyLogView({ v }: { v: DailyLogVals }) {
  return (
    <>
      <div data-shell style={{ minHeight: '100vh', fontFamily: 'var(--font-body)', fontWeight: '300', color: 'var(--ink-soft)', background: '#EDF1F8' }}>
      ...
      <span style={{ fontFamily: 'var(--font-display)', fontSize: '19px', color: 'var(--ink)', letterSpacing: '-0.015em' }}>Daily<em style={{ color: '#17365D' }}> log</em></span>
      ...
      <span style={{ display: 'inline-flex', alignItems: 'center', gap: '7px', padding: '6px 12px', borderRadius: '999px', background: '#E9EFF7', border: '0.5px solid rgba(23,54,93,0.20)', fontSize: '10.5px', color: '#7A3D91' }}>
      <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: v.syncDotColour, display: 'inline-block' }}></span>
      {v.syncLabel}
      </span>
      </div>
      </div>
      <main data-main style={{ padding: '20px 48px 88px', maxWidth: '1360px', minWidth: '0', margin: '0 auto' }}>
      <div data-body style={{ display: 'grid', gridTemplateColumns: '250px minmax(0,1fr)', gap: '20px', alignItems: 'start' }}>
      <aside data-rail style={{ position: 'sticky', top: '20px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
      <div style={{ background: '#FFFFFF', border: '1px solid rgba(27,39,51,0.12)', borderRadius: '8px', overflow: 'hidden' }}>
      <div style={{ padding: '22px 20px', textAlign: 'center', borderBottom: '1px solid rgba(27,39,51,0.10)' }}>
      <div style={{ width: '76px', height: '76px', borderRadius: '50%', background: '#8E4BA8', color: '#FFFFFF', display: 'grid', placeItems: 'center', margin: '0 auto 12px', fontFamily: 'var(--font-display)', fontSize: '28px', letterSpacing: '0.02em' }}>{v.profile.initials}</div>
      <div style={{ fontSize: '15px', color: 'var(--ink)', marginBottom: '3px' }}>{v.profile.name}</div>
      <div style={{ fontSize: '11px', letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--ink-mute)' }}>{v.profile.phase}</div>
      </div>
      <div style={{ padding: '22px 20px', borderBottom: '1px solid rgba(27,39,51,0.10)' }}>
      <svg viewBox='0 0 180 180' style={{ width: '100%', maxWidth: '168px', display: 'block', margin: '0 auto' }}>
      <circle cx='90' cy='90' r='76' fill='none' stroke='#E1E7F0' strokeWidth='12'></circle>
      <circle cx='90' cy='90' r='76' fill='none' stroke='#17365D' strokeWidth='12' strokeLinecap='round' strokeDasharray={v.profile.goalRing.dash} strokeDashoffset={v.profile.goalRing.offset} transform='rotate(-90 90 90)' style={{ transition: 'stroke-dashoffset 700ms cubic-bezier(.16,1,.3,1)' }}></circle>
      <text x='90' y='88' textAnchor='middle' fontFamily='var(--font-display)' fontSize='30' fill='#1B2733'>{v.profile.goalRing.value}<tspan fontSize='17' fill='#6E7A88'>/{v.profile.goalRing.total}</tspan></text>
      <text x='90' y='108' textAnchor='middle' fontSize='10' letterSpacing='2' fill='#6E7A88'>{v.profile.goalRing.unit.toUpperCase()}</text>
      </svg>
      </div>
      <div style={{ padding: '6px 20px 16px' }}>
      {(v.profile.fields ?? []).map((f, f_i) => (<React.Fragment key={f_i}>
      <div style={{ padding: '11px 0', borderBottom: f_i === (v.profile.fields.length - 1) ? '0' : '1px solid rgba(27,39,51,0.08)' }}>
      <div style={{ fontSize: '10px', letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--ink-mute)', marginBottom: '3px' }}>{f.label}</div>
      <div style={{ fontSize: '13.5px', color: 'var(--ink)' }}>{f.value}</div>
      </div>
      </React.Fragment>))}
      </div>
      </div>
      </aside>
      <div data-col style={{ minWidth: '0' }}>
      <header style={{ marginBottom: '20px' }}>
      <div style={{ fontSize: '11px', letterSpacing: '0.18em', textTransform: 'uppercase', color: '#6E7A88', marginBottom: '8px' }}>Fitness &amp; recovery</div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '24px', flexWrap: 'wrap' }}>
      <h1 style={{ fontFamily: 'var(--font-display)', fontWeight: '400', fontSize: '42px', letterSpacing: '-0.025em', lineHeight: '1.06', color: 'var(--ink)', margin: '0' }}>{v.greetingLead} <em style={{ fontStyle: 'italic', color: '#17365D' }}>{v.greetingTail}</em></h1>
      ...
      <p style={{ margin: '10px 0 0', fontSize: '14.5px', fontWeight: '300', color: '#6E7A88', lineHeight: '1.6', maxWidth: '58ch' }}>{v.subhead}</p>
      </header>
      <div style={{ height: '0.5px', background: 'rgba(27,39,51,0.14)', margin: '0 0 26px' }}></div>
      {(v.isFresh) ? (<>
      <div style={{ padding: '40px 42px', background: '#FFFFFF', border: '0.5px solid rgba(27,39,51,0.14)', borderRadius: '12px', boxShadow: 'none', maxWidth: '760px' }}>
      <span style={{ fontSize: '10px', letterSpacing: '0.18em', textTransform: 'uppercase', color: '#7A3D91' }}>Day one</span>
      <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '34px', color: 'var(--ink)', margin: '12px 0 10px', letterSpacing: '-0.02em', lineHeight: '1.12' }}>Nothing logged yet — <em style={{ color: '#17365D' }}>let&apos;s fix that</em>.</h2>
      <p style={{ margin: '0 0 26px', fontSize: '14px', color: '#6E7A88', lineHeight: '1.7', maxWidth: '56ch' }}>Three things and the dashboard comes alive: connect Apple Health so weight and activity arrive on their own, set the pace you want to lose at, then log today&apos;s first meal.</p>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
      {(v.onboardSteps ?? []).map((s, s_i) => (<React.Fragment key={s_i}>
      <div style={{ display: 'grid', gridTemplateColumns: '30px 1fr auto', gap: '16px', alignItems: 'center', padding: '16px 18px', borderRadius: '10px', background: '#FFFFFF', border: '0.5px solid rgba(178,152,106,0.14)' }}>
      <span style={{ fontFamily: 'var(--font-display)', fontSize: '19px', color: '#17365D' }}>{s.num}</span>
      <div>
      <div style={{ fontSize: '13.5px', color: 'var(--ink)' }}>{s.title}</div>
      <div style={{ fontSize: '11.5px', color: '#6E7A88', marginTop: '2px' }}>{s.note}</div>
      </div>
      ...
      {(v.hasData) ? (<>
      <div data-hero style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.1fr) minmax(0,1fr)', gap: '36px', alignItems: 'end', padding: '30px 34px', marginBottom: '16px', background: '#FFFFFF', border: '0.5px solid rgba(27,39,51,0.14)', borderRadius: '12px', boxShadow: 'none' }}>
      <div>
      <div style={{ fontSize: '10px', letterSpacing: '0.18em', textTransform: 'uppercase', color: '#7A3D91', marginBottom: '12px' }}>This morning</div>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: '12px' }}>
      ...
      <span style={{ fontFamily: 'var(--font-display)', fontStyle: 'italic', fontSize: '26px', color: '#8592A1' }}>{v.unitLabel}</span>
      ...
      <p style={{ margin: '16px 0 0', fontSize: '14px', color: '#6E7A88', lineHeight: '1.6', maxWidth: '44ch' }}>{v.heroVerdict}</p>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: '14px', paddingTop: '14px', borderTop: '0.5px solid rgba(27,39,51,0.13)' }}>
      {(v.heroStats ?? []).map((s, s_i) => (<React.Fragment key={s_i}>
      <div>
      <div style={{ fontSize: '9.5px', letterSpacing: '0.16em', textTransform: 'uppercase', color: '#8592A1' }}>{s.label}</div>
      <div style={{ fontFamily: 'var(--font-display)', fontSize: '20px', color: 'var(--ink)', marginTop: '4px' }}>{s.value}</div>
      <div style={{ fontSize: '10.5px', color: '#6E7A88' }}>{s.note}</div>
      </div>
      </React.Fragment>))}
      </div>
      </div>
      ...
      {(v.targetsOpen) ? (<>
      <div data-cols-2 style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '14px', padding: '22px 26px', marginBottom: '16px', background: '#FFFFFF', border: '0.5px solid rgba(27,39,51,0.14)', borderRadius: '12px' }}>
      {(v.targetFields ?? []).map((t, t_i) => (<React.Fragment key={t_i}>
      <div>
      <div style={{ fontSize: '9.5px', letterSpacing: '0.16em', textTransform: 'uppercase', color: '#8592A1', marginBottom: '8px' }}>{t.label}</div>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: '7px' }}>
      <input value={t.value} onChange={t.onChange} type='number' step={t.step} style={{ width: '88px', padding: '9px 12px', border: '0.5px solid rgba(27,39,51,0.10)', borderRadius: '12px', background: '#FFFFFF', fontFamily: 'var(--font-display)', fontSize: '18px', color: 'var(--ink)', outline: 'none' }} style-focus='border-color:#8FB6DE;' />
      <span style={{ fontSize: '11px', color: '#6E7A88' }}>{t.unit}</span>
      </div>
      </div>
      </React.Fragment>))}
      </div>
      </>) : null}
      <div data-metric-band style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(178px,1fr))', background: '#F7F9FC', border: '0.5px solid rgba(27,39,51,0.13)', borderRadius: '12px', overflow: 'hidden', marginBottom: '26px' }}>
      {(v.kpis ?? []).map((k, k_i) => (<React.Fragment key={k_i}>
      <div data-metric-cell style={{ padding: '20px 22px 16px', borderLeft: k_i === 0 ? '0' : '0.5px solid rgba(27,39,51,0.11)' }}>
      ...
      <div style={{ padding: '24px 26px', background: '#FFFFFF', border: '0.5px solid rgba(27,39,51,0.14)', borderRadius: '12px', boxShadow: 'none' }}>
      ...
      <text x='100' y='96' textAnchor='middle' fontFamily='Playfair Display' fontSize='30' fill='#1B2733'>{v.ringFocus.value}</text>
      <text x='100' y='116' textAnchor='middle' fontFamily='Inter' fontSize='9' letterSpacing='2' fill='#6E7A88'>{v.ringFocus.label}</text>
      </svg>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
      {(v.rings ?? []).map((r, r_i) => (<React.Fragment key={r_i}>
      ...
      <div style={{ padding: '22px 24px', background: '#FFFFFF', border: '0.5px solid rgba(27,39,51,0.14)', borderRadius: '12px', boxShadow: 'none' }}>
      ...
      <input value={v.draftWeight} onChange={v.onDraftChange} onKeyDown={v.onDraftKey} placeholder='71.4' inputMode='decimal' style={{ flex: '1', padding: '12px 14px', border: '0.5px solid var(--rule)', borderRadius: '8px', background: 'var(--paper)', fontFamily: 'var(--font-display)', fontSize: '20px', color: 'var(--ink)', outline: 'none' }} style-focus='border-color:#C9DCEF;' />
      ...
      <div style={{ padding: '22px 24px', background: '#FFFFFF', border: '0.5px solid rgba(23,54,93,0.16)', borderRadius: '12px' }}>
      ...
      <div>
      <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '22px', color: 'var(--ink)', margin: '0 0 5px', letterSpacing: '-0.015em' }}>Weight · <em style={{ color: '#17365D' }}>{v.rangeLabel}</em></h2>
      <p style={{ margin: '0', fontSize: '12.5px', color: '#6E7A88' }}>{v.chartSub}</p>
      </div>
      <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
      {(v.chartStats ?? []).map((c, c_i) => (<React.Fragment key={c_i}>
      <div>
      <div style={{ fontSize: '9.5px', letterSpacing: '0.16em', textTransform: 'uppercase', color: '#8592A1' }}>{c.label}</div>
      <div style={{ fontFamily: 'var(--font-display)', fontSize: '21px', color: 'var(--ink)', letterSpacing: '-0.015em', lineHeight: '1.25' }}>{c.value}</div>
      <div style={{ fontSize: '10.5px', color: '#8592A1' }}>{c.unit}</div>
      </div>
      </React.Fragment>))}
      </div>
      </div>
      ...
      <g>
      <line x1='54' x2='846' y1={t.y} y2={t.y} stroke='rgba(27,39,51,0.07)' strokeWidth='0.5'></line>
      <text x='44' y={t.ty} textAnchor='end' fontFamily='Inter' fontSize='10' fill='#8592A1'>{t.label}</text>
      </g>
      </React.Fragment>))}
      <line x1='54' x2='846' y1={v.goalY} y2={v.goalY} stroke='#8E4BA8' strokeWidth='1.2' strokeDasharray='5 5'></line>
      <text x='58' y={v.goalTextY} fontFamily='Inter' fontSize='9.5' letterSpacing='1.6' fill='#A46BBA'>{v.goalChip}</text>
      <path d={v.bandPath} fill='#C9DCEF' opacity='0.42'></path>
      <path d={v.projBandPath} fill='#B5E0D8' opacity='0.34'></path>
      {(v.rawDots ?? []).map((d, d_i) => (<React.Fragment key={d_i}>
      <circle cx={d.x} cy={d.y} r='1.9' fill='#8E4BA8' opacity='0.62'></circle>
      </React.Fragment>))}
      <defs>
      <linearGradient id='trend-fill' x1='0' y1='0' x2='0' y2='1'>
      <stop offset='0%' stopColor='#17365D' stopOpacity='0.18'></stop>
      <stop offset='100%' stopColor='#17365D' stopOpacity='0'></stop>
      </linearGradient>
      </defs>
      <path d={`${v.maPath} L846,286 L54,286 Z`} fill='url(#trend-fill)' stroke='none'></path>
      <path d={v.maPath} fill='none' stroke='#17365D' strokeWidth='2.6' strokeLinejoin='round' strokeLinecap='round'></path>
      ...
      <text x={v.projLabelX} y={v.projLabelY} textAnchor='end' fontFamily='Playfair Display' fontStyle='italic' fontSize='15' fill='#3E9C8C'>{v.projLabel}</text>
      {(v.weighDots ?? []).map((p, p_i) => (<React.Fragment key={p_i}>
      <circle cx={p.x} cy={p.y} r={p.r} fill='#FFFFFF' stroke='#17365D' strokeWidth='1.8'></circle>
      </React.Fragment>))}
      <g>
      <circle cx={v.loMarker.x} cy={v.loMarker.y} r='3' fill='#3E9C8C'></circle>
      <text x={v.loMarker.x} y={v.loMarker.ly} textAnchor='middle' fontFamily='Inter' fontSize='9' letterSpacing='1.2' fill='#3E9C8C'>{v.loMarker.label}</text>
      <circle cx={v.hiMarker.x} cy={v.hiMarker.y} r='3' fill='#A46BBA'></circle>
      <text x={v.hiMarker.x} y={v.hiMarker.ly} textAnchor='middle' fontFamily='Inter' fontSize='9' letterSpacing='1.2' fill='#A46BBA'>{v.hiMarker.label}</text>
      </g>
      {(v.crosshair) ? (<>
      <g>
      <line x1={v.crosshair.x} x2={v.crosshair.x} y1='22' y2='286' stroke='#17365D' strokeWidth='0.75' strokeDasharray='3 4' opacity='0.55'></line>
      <circle cx={v.crosshair.x} cy={v.crosshair.y} r='5.5' fill='#17365D'></circle>
      <circle cx={v.crosshair.x} cy={v.crosshair.y} r='10' fill='none' stroke='#17365D' strokeWidth='0.8' opacity='0.4'></circle>
      </g>
      </>) : null}
      ...
      <div style={{ display: 'flex', gap: '22px', flexWrap: 'wrap', marginTop: '14px', paddingTop: '14px', borderTop: '0.5px solid rgba(27,39,51,0.13)', fontSize: '11px', color: '#6E7A88' }}>
      <span style={{ display: 'inline-flex', alignItems: 'center', gap: '7px' }}><i style={{ width: '9px', height: '9px', borderRadius: '50%', background: '#8E4BA8', display: 'inline-block' }}></i> Daily scale reading</span>
      <span style={{ display: 'inline-flex', alignItems: 'center', gap: '7px' }}><i style={{ width: '16px', height: '2.5px', borderRadius: '2px', background: '#17365D', display: 'inline-block' }}></i> 7-day trend</span>
      <span style={{ display: 'inline-flex', alignItems: 'center', gap: '7px' }}><i style={{ width: '14px', height: '9px', borderRadius: '3px', background: '#C9DCEF', display: 'inline-block' }}></i> Daily fluctuation band</span>
      ...
      <span style={{ display: 'inline-flex', alignItems: 'center', gap: '7px' }}><i style={{ width: '16px', height: '2.5px', borderRadius: '2px', background: '#8E4BA8', display: 'inline-block' }}></i> Goal</span>
      </div>
      </div>
      ...
      <StatRowHover sparkPoints={b.spark} color='#17365D'>
      ...
      {(v.isPlan) ? (<>
      <div data-tab-panel style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      <div style={{ padding: '30px 32px', background: '#FFFFFF', border: '0.5px solid rgba(27,39,51,0.14)', borderRadius: '12px', boxShadow: 'none' }}>
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: '20px', flexWrap: 'wrap', marginBottom: '22px' }}>
      <div>
      <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '24px', color: 'var(--ink)', margin: '0 0 5px', letterSpacing: '-0.015em' }}>Your plan · <em style={{ color: '#17365D' }}>{v.paceLabel} a week</em></h2>
      <p style={{ margin: '0', fontSize: '12.5px', color: '#6E7A88', maxWidth: '64ch' }}>{v.planSub}</p>
      </div>
      ...
      <div style={{ fontSize: '9.5px', letterSpacing: '0.16em', textTransform: 'uppercase', color: '#6E7A88' }}>{n.label}</div>
      ...
      <div style={{ fontSize: '11px', color: '#6E7A88', marginTop: '6px' }}>{n.note}</div>
      </div>
      </React.Fragment>))}
      </div>
      <p style={{ margin: '18px 0 0', fontSize: '12.5px', color: '#6E7A88', lineHeight: '1.7', maxWidth: '82ch' }}>{v.planCaution}</p>
      </div>
      <div data-cols style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) minmax(0,1fr)', gap: '16px', alignItems: 'start' }}>
      <div style={{ padding: '26px 28px', background: '#FFFFFF', border: '0.5px solid rgba(27,39,51,0.14)', borderRadius: '12px', boxShadow: 'none' }}>
      ...
      <p style={{ margin: '0 0 16px', fontSize: '12.5px', color: '#6E7A88' }}>Maintenance back-calculated from what you actually ate and what the scale actually did — not the formula.</p>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '14px' }}>
      {(v.realityNumbers ?? []).map((r, r_i) => (<React.Fragment key={r_i}>
      <div style={{ padding: '16px 18px', borderRadius: '10px', background: '#FFFFFF', border: '0.5px solid rgba(178,152,106,0.14)' }}>
      <div style={{ fontSize: '9.5px', letterSpacing: '0.16em', textTransform: 'uppercase', color: '#8592A1' }}>{r.label}</div>
      <div style={{ fontFamily: 'var(--font-display)', fontSize: '24px', color: 'var(--ink)', marginTop: '6px', lineHeight: '1' }}>{r.value}</div>
      <div style={{ fontSize: '10.5px', color: '#6E7A88', marginTop: '5px' }}>{r.note}</div>
      </div>
      </React.Fragment>))}
      </div>
      <p style={{ margin: '16px 0 0', fontSize: '13px', color: '#46525F', lineHeight: '1.7' }}>{v.realityCopy2}</p>
      </div>
      <div style={{ padding: '26px 28px', background: '#FFFFFF', border: '0.5px solid rgba(27,39,51,0.14)', borderRadius: '12px', boxShadow: 'none' }}>
      ...
      <span style={{ fontSize: '10.5px', padding: '5px 12px', borderRadius: '999px', background: '#E6EEF8', color: '#0F2745' }}>{v.adherenceLabel}</span>
      </div>
      {(v.adherenceRows ?? []).map((a, a_i) => (<React.Fragment key={a_i}>
      <div style={{ display: 'grid', gridTemplateColumns: '130px 1fr auto', gap: '12px', alignItems: 'center', padding: '10px 0', borderTop: '0.5px solid rgba(27,39,51,0.13)' }}>
      <span style={{ fontSize: '12.5px', color: '#46525F' }}>{a.label}</span>
      <div style={{ height: '9px', borderRadius: '999px', background: '#E4F3F0', overflow: 'hidden' }}><div style={sx(a.barStyle)}></div></div>
      <span style={sx(`font-family:var(--font-display);font-size:16px;color:${a.color};min-width:56px;text-align:right;`)}>{a.value}</span>
      </div>
      </React.Fragment>))}
      <p style={{ margin: '16px 0 0', fontSize: '13px', color: '#46525F', lineHeight: '1.7' }}>{v.adherenceCopy}</p>
      </div>
      </div>
      ...
      <div>
      <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '19px', color: 'var(--ink)', margin: '0 0 4px' }}>Smart <em style={{ color: '#17365D' }}>scheduling</em></h2>
      <p style={{ margin: '0', fontSize: '12.5px', color: '#6E7A88', maxWidth: '46ch' }}>{v.scheduleCopy}</p>
      </div>
      <span style={sx(v.scheduleBadgeStyle)}>{v.scheduleBadgeLabel}</span>
      </div>
      {(v.scheduleRows ?? []).map((n, n_i) => (<React.Fragment key={n_i}>
      <div style={{ display: 'grid', gridTemplateColumns: '52px 1fr 1fr', gap: '14px', alignItems: 'center', padding: '11px 0', borderTop: '0.5px solid rgba(27,39,51,0.13)' }}>
      <span style={{ fontSize: '10.5px', letterSpacing: '0.12em', textTransform: 'uppercase', color: '#8592A1' }}>{n.day}</span>
      ...
      <p style={{ margin: '0 0 14px', fontSize: '12.5px', color: '#6E7A88' }}>Three lifts, two easy days — enough stimulus to hold muscle while the deficit runs.</p>
      {(v.planWeek ?? []).map((d, d_i) => (<React.Fragment key={d_i}>
      <div style={{ display: 'grid', gridTemplateColumns: '46px 1fr auto', gap: '14px', alignItems: 'center', padding: '13px 0', borderTop: '0.5px solid rgba(27,39,51,0.13)' }}>
      <span style={{ fontSize: '10.5px', letterSpacing: '0.12em', textTransform: 'uppercase', color: '#8592A1' }}>{d.day}</span>
      <div>
      <div style={{ fontSize: '13.5px', color: 'var(--ink)' }}>{d.session}</div>
      <div style={{ fontSize: '11.5px', color: '#6E7A88', marginTop: '2px' }}>{d.detail}</div>
      </div>
      ...
      <span style={{ fontSize: '12.5px', color: '#46525F' }}>{m.label}</span>
      <span style={{ fontFamily: 'var(--font-display)', fontSize: '17px', color: 'var(--ink)' }}>{m.value}</span>
      </div>
      <div style={{ height: '8px', borderRadius: '999px', background: '#E4F3F0', overflow: 'hidden' }}>
      <div style={sx(m.barStyle)}></div>
      </div>
      <div style={{ fontSize: '11px', color: '#6E7A88', marginTop: '6px' }}>{m.note}</div>
      </div>
      </React.Fragment>))}
      </div>
      <div style={{ padding: '26px 28px', background: '#FFFFFF', border: '0.5px solid rgba(23,54,93,0.16)', borderRadius: '12px' }}>
      <span style={{ fontSize: '10px', letterSpacing: '0.16em', textTransform: 'uppercase', color: '#6E7A88' }}>Non-negotiables</span>
      ...
      <span style={{ width: '7px', height: '7px', borderRadius: '50%', background: '#8E4BA8', marginTop: '7px', flex: 'none' }}></span>
      <span style={{ fontSize: '13px', color: '#46525F', lineHeight: '1.55' }}>{r}</span>
      </div>
      </React.Fragment>))}
      </div>
      </div>
      ...
      <p style={{ margin: '0', fontSize: '12.5px', color: '#6E7A88' }}>Tick a week off as you weigh in — the bar fills as you go.</p>
      </div>
      <span style={{ fontSize: '10.5px', padding: '6px 13px', borderRadius: '999px', background: '#E6EEF8', color: '#0F2745' }}>{v.planDoneLabel}</span>
      </div>
      ...
      <div>
      <div style={{ fontSize: '10px', letterSpacing: '0.18em', textTransform: 'uppercase', color: '#8592A1', marginBottom: '10px' }}>Lost since February</div>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: '8px' }}>
      <span style={{ fontFamily: 'var(--font-display)', fontSize: '76px', lineHeight: '0.9', letterSpacing: '-0.03em', color: '#17365D' }}>
      <TickerNumber value={v.lostValue} decimals={1}></TickerNumber>
      </span>
      <span style={{ fontFamily: 'var(--font-display)', fontStyle: 'italic', fontSize: '24px', color: '#8592A1' }}>{v.unitLabel}</span>
      </div>
      <p style={{ margin: '10px 0 0', fontSize: '13px', color: '#6E7A88', maxWidth: '26ch' }}>{v.lostCopy}</p>
      </div>
      <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px', color: '#8592A1', marginBottom: '10px' }}>
      <span>{v.startLabel}</span>
      <span>{v.pctLabel} of the way to {v.goalDisplay}</span>
      </div>
      <div style={{ position: 'relative', height: '14px', borderRadius: '999px', background: '#E4F3F0', overflow: 'hidden' }}>
      ...
      <div>
      <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '22px', color: 'var(--ink)', margin: '0 0 5px', letterSpacing: '-0.015em' }}>Progress <em style={{ color: '#8E4BA8' }}>pictures</em></h2>
      <p style={{ margin: '0', fontSize: '12.5px', color: '#6E7A88' }}>Drop a photo into any frame — same light, same time of day, and the comparison actually means something.</p>
      </div>
      <span style={{ fontSize: '10.5px', padding: '6px 13px', borderRadius: '999px', background: '#E9EFF7', border: '0.5px solid rgba(23,54,93,0.16)', color: '#7A3D91' }}>private to you</span>
      </div>
      ...
      <div>
      <div style={{ position: 'relative', width: '100%', aspectRatio: '3/4', borderRadius: '12px', overflow: 'hidden', background: '#FFFFFF', border: '0.5px solid rgba(178,152,106,0.14)' }}>
      ...
      <span style={{ fontSize: '11.5px', color: '#6E7A88' }}>{p.date}</span>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '78px 1fr auto', gap: '14px', alignItems: 'center', padding: '11px 0', borderTop: '0.5px solid rgba(27,39,51,0.13)' }}>
      <span style={{ fontSize: '12.5px', color: '#6E7A88' }}>{m.month}</span>
      <div style={{ height: '10px', borderRadius: '999px', background: '#E4F3F0', overflow: 'hidden' }}>
      <div style={sx(m.barStyle)}></div>
      </div>
      ...
      <div style={{ padding: '26px 28px', background: '#FFFFFF', border: '0.5px solid rgba(23,54,93,0.16)', borderRadius: '12px' }}>
      <span style={{ fontSize: '10px', letterSpacing: '0.16em', textTransform: 'uppercase', color: '#6E7A88' }}>Non-scale wins</span>
      ...
      <div style={{ fontSize: '11.5px', color: '#6E7A88', marginTop: '2px' }}>{w.note}</div>
      </div>
      </div>
      </React.Fragment>))}
      </div>
      ...
      <div style={{ padding: '24px 26px', marginBottom: '16px', background: '#FFFFFF', border: '0.5px solid rgba(27,39,51,0.14)', borderRadius: '12px', boxShadow: 'none' }}>
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: '16px', marginBottom: '2px' }}>
      <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '18px', color: 'var(--ink)', margin: '0' }}>Weekly <em style={{ color: '#17365D' }}>deficit</em></h2>
      ...
      <div style={{ padding: '24px 26px', marginBottom: '16px', background: '#FFFFFF', border: '0.5px solid rgba(27,39,51,0.14)', borderRadius: '12px' }}>
      <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '18px', color: 'var(--ink)', margin: '0 0 2px' }}>What the numbers <em style={{ color: '#17365D' }}>say</em></h2>
      ...
      <span style={sx(`font-size:10px;letter-spacing:0.14em;text-transform:uppercase;padding:5px 10px;border-radius:999px;text-align:center;background:${c.tone === 'ok' ? '#E4F3F0' : c.tone === 'mid' ? '#E6EEF8' : '#F3ECF7'};color:${c.tone === 'ok' ? '#3E9C8C' : c.tone === 'mid' ? '#0F2745' : '#C0492F'};`)}>{c.tag}</span>
      <div>
      ...
      <div style={{ padding: '8px 26px 4px', background: '#FFFFFF', border: '0.5px solid rgba(27,39,51,0.14)', borderRadius: '12px', boxShadow: 'none' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: '20px', flexWrap: 'wrap', marginBottom: '22px' }}>
      <div>
      <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '22px', color: 'var(--ink)', margin: '0 0 5px', letterSpacing: '-0.015em' }}>{v.volTitleLead} <em style={{ color: '#17365D' }}>{v.volTitleTail}</em></h2>
      <p style={{ margin: '0', fontSize: '12.5px', color: '#6E7A88' }}>{v.volCaption}</p>
      </div>
      <div style={{ display: 'flex', gap: '8px' }}>
      ...
      <span style={{ fontFamily: 'var(--font-display)', fontStyle: 'italic', fontSize: '22px', color: '#8592A1' }}>{v.volUnit}</span>
      </div>
      <div style={{ fontSize: '12px', color: '#6E7A88', marginTop: '8px' }}>{v.volSub}</div>
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: '18px' }}>
      <div style={{ width: '96px', height: '96px', borderRadius: '50%', background: '#E9EFF7', border: '0.5px solid rgba(23,54,93,0.22)', flex: 'none', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '2px' }}>
      <span style={{ fontFamily: 'var(--font-display)', fontSize: '22px', color: '#17365D', letterSpacing: '-0.02em' }}>{v.animalUnitKg}</span>
      <span style={{ fontSize: '9px', letterSpacing: '0.16em', textTransform: 'uppercase', color: '#7A3D91' }}>{v.volUnitNote}</span>
      </div>
      <div>
      <div style={{ fontSize: '10px', letterSpacing: '0.16em', textTransform: 'uppercase', color: '#7A3D91' }}>that is about</div>
      ...
      <div style={{ fontFamily: 'var(--font-display)', fontStyle: 'italic', fontSize: '17px', color: '#17365D' }}>{v.animalName}</div>
      <div style={{ fontSize: '11px', color: '#6E7A88', marginTop: '4px' }}>{v.animalNote}</div>
      </div>
      </div>
      <div>
      ...
      <span style={sx(`width:34px;height:34px;border-radius:12px;background:${s.tint};display:grid;place-items:center;font-size:11px;letter-spacing:0.08em;color:#7A3D91;`)}>{s.day}</span>
      <span>
      ...
      <div style={{ padding: '24px 26px', background: '#FFFFFF', border: '0.5px solid rgba(23,54,93,0.16)', borderRadius: '12px' }}>
      ...
      {(v.liftProgress) ? (<>
      <div style={{ padding: '24px 26px', marginTop: '16px', background: '#FFFFFF', border: '0.5px solid rgba(27,39,51,0.14)', borderRadius: '12px' }}>
      <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '18px', color: 'var(--ink)', margin: '0 0 2px' }}>Where each lift <em style={{ color: '#17365D' }}>is going</em></h2>
      ...
      <div style={{ padding: '14px 16px', background: '#F7F9FC', border: '0.5px solid rgba(27,39,51,0.11)', borderRadius: '10px' }}>
      ...
      <stop offset='0%' stopColor='#17365D' stopOpacity='0.22'></stop>
      <stop offset='100%' stopColor='#17365D' stopOpacity='0'></stop>
      </linearGradient>
      </defs>
      <path d={l.area} fill={`url(#${l.gradId})`} stroke='none'></path>
      <path d={l.path} fill='none' stroke='#17365D' strokeWidth='2' strokeLinejoin='round' strokeLinecap='round' vectorEffect='non-scaling-stroke'></path>
      {(l.prs ?? []).map((q, q_i) => (<React.Fragment key={q_i}>
      <circle cx={q.x} cy={q.y} r='3.4' fill='#FFFFFF' stroke='#17365D' strokeWidth='1.6' vectorEffect='non-scaling-stroke'></circle>
      </React.Fragment>))}
      </svg>
      ...
      <span style={{ fontSize: '11px', padding: '5px 12px', borderRadius: '999px', background: '#E4F3F0', color: 'var(--green)' }}>{v.habitPct}% complete</span>
      </div>
