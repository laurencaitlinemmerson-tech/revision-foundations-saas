    ...
    <div className="min-h-screen bg-cream">
      ...
      <section className="bg-cream section">
        <div className="max-w-6xl mx-auto px-6 text-center">
          <span className="badge badge-purple mb-4 inline-flex animate-on-scroll badge-shimmer">Why Us?</span>
          <h2 className="mb-12 text-[var(--plum-dark)] animate-on-scroll">Made With Love</h2>
          <div className="grid gap-10 md:grid-cols-3">
            {[
              { icon: '👩‍🎓', title: 'By a Student', copy: 'Created by a nursing student who knows exactly what you need.' },
              { icon: '⚡', title: 'Easy to Use', copy: 'Simple, pretty interface that makes revision actually fun.' },
              { icon: '🎁', title: 'Pay Once', copy: 'One-time payment. Lifetime access. No subscription.' },
            ].map((item, i) => (
              <div key={item.title} className="space-y-3 animate-on-scroll fade-in-up" style={{ animationDelay: `${i * 0.1}s` }}>
                <div className="text-4xl emoji-float" style={{ animationDelay: `${i * 0.3}s` }}>{item.icon}</div>
      ...
      <section className="gradient-hero section relative overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto px-6 text-center relative z-10">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center animate-on-scroll">
            <Link href="/pricing" className="btn-primary btn-hover px-8 py-4 bg-white text-[var(--purple)] hover:bg-white/90">
              <Sparkles className="w-5 h-5" />
              Get Started – £4.99 one-time
            </Link>
            <Link href="/quiz" className="btn-secondary btn-hover px-8 py-4 bg-white/20 text-white border-white/30 hover:bg-white/30">
