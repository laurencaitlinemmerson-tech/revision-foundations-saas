...
import HubCard, { HubItem } from '@/components/HubCard';
...
export default function HubPage() {
  ...
  // TODO: Replace with real subscription check when Stripe is wired
  // For now, defaults to free - connect to getUserEntitlements later
  const isPro = false;
            ...
            {filteredItems.length === 0 ? (
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredItems.map((item) => (
                  <HubCard key={item.id} item={item} isPro={isPro} />
                ))}
              </div>
            )}
          </div>
